...
export interface ArticleActions {
  [ArticleActionTypes.FETCH_ARTICLE](context: AugmentedActionContext, articleSlug: string, prevArticle: Article): void
...
const actions: ActionTree<ArticleStateInterface, StateInterface> & ArticleActions = {
   [ArticleActionTypes.FETCH_ARTICLE](context, articleSlug, prevArticle) {
    // avoid extronuous network call if article exists
    if (prevArticle !== undefined) {
      // return context.commit(ArticleMutationTypes.SET_ARTICLE, prevArticle)
    }
    // const {data} =  ArticlesService.get(articleSlug)
